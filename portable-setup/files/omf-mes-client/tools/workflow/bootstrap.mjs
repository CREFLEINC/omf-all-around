...
export const INTEGRATION_SOURCE = 'docs/client-dev-workflow/integration.md';
const INTEGRATION_MARKER = '<!-- omf-integration: phase3-1 -->';

export function integrationEnabled(root) {
  return Object.values(ADAPTERS).some(({ file }) => {
    const target = path.join(root, file);
    return existsSync(target) && readFileSync(target, 'utf8').includes(INTEGRATION_MARKER);
  });
}

function integrationAdapter(root, tool) {
  return `# ${ADAPTERS[tool].title} 통합 담당\n\n${INTEGRATION_MARKER}\n\n이 파일은 ${INTEGRATION_SOURCE}에서 생성한다. 직접 편집하지 않는다.\n\n${readFileSync(path.join(root, INTEGRATION_SOURCE), 'utf8')}`;
}

function bootstrapIntegration(root, options) {
  const writes = selectedTools(options.tool).map((tool) => {
    const target = path.join(root, ADAPTERS[tool].file);
    const content = integrationAdapter(root, tool);
    if (existsSync(target)) {
      const previous = readFileSync(target, 'utf8');
      if (!previous.includes(INTEGRATION_MARKER) && !options.force)
        throw new Error(
          '기존 어댑터를 먼저 백업하세요. 통합 모드로 교체하려면 --force가 필요합니다.',
        );
    }
    return { target, content };
  });
  for (const { target, content } of writes) writeFileSync(target, content);
  process.stdout.write('integration adapters ready: phase3-1\n');
}
...
export function bootstrapErrors(root) {
  if (integrationEnabled(root)) {
    const errors = [];
    for (const [tool, { file }] of Object.entries(ADAPTERS)) {
      const target = path.join(root, file);
      if (!existsSync(target)) continue;
      try {
        if (readFileSync(target, 'utf8') !== integrationAdapter(root, tool))
          errors.push(`${file}: 통합 지침과 다릅니다. --mode integration으로 함께 재생성하세요.`);
      } catch (error) {
...
export function bootstrap(root, options) {
  if (options.mode === 'integration') return bootstrapIntegration(root, options);
  if (options.mode && options.mode !== 'legacy') throw new Error('지원하지 않는 mode입니다.');
  if (integrationEnabled(root))
    throw new Error(
      '통합 모드가 활성화되어 있습니다. --mode integration을 사용하세요. 종전 모드 복원은 백업으로 수행하세요.',
    );
...
function usage() {
  return `사용법:
  pnpm workflow:bootstrap --mode integration --tool <codex|claude|both>
